...
class Degeration(nn.Module):
    def __init__(self, hr_size=256, srf=4, usm=False, need_l1=False):
        super(Degeration, self).__init__()
        self.hr_size = hr_size
        self.srf = srf
        self.usm = usm
        self.need_l1 = need_l1
    ...
    # ----------------------- The first degradation process ----------------------- #
        ...
        cur_first = current_lr.detach().clone()
        mode = random.choice(['area', 'bilinear', 'bicubic'])
        cur_first = F.interpolate(cur_first, size=(ori_h // self.srf, ori_w // self.srf), mode=mode)
        cur_first = torch.clamp((cur_first * 255.0).round(), 0, 255) / 255.
    ...
    # ----------------------- The second degradation process ----------------------- #
        ...
        # get final data
        cur_full = torch.clamp((current_lr * 255.0).round(), 0, 255) / 255.

    # ----------------------- Blur only ----------------------- #
        # keep blur/resizing/sinc; DROP noise & JPEG entirely
        if self.usm:
            cur_blur = filter2D(hr_usm, self.kernel_1)
        else:
            cur_blur = filter2D(hr, self.kernel_1)

        # same resize branch as stage-1 (but no noise/JPEG)
        cur_blur = self._resize_img(cur_blur, self.resize_prob, self.resize_range)

        # optional second blur
        if np.random.uniform() < self.second_blur_prob:
            cur_blur = filter2D(cur_blur, self.kernel_2)

        # second resize (no noise/JPEG)
        cur_blur = self._resize_img(cur_blur, self.resize_prob2, self.resize_range2)

        # final sinc branch, no JPEG
        if np.random.uniform() < 0.5:
            cur_blur = self._jpge_compress(cur_blur, self.jpeg_range)
            mode = random.choice(['area', 'bilinear', 'bicubic'])
            cur_blur = F.interpolate(cur_blur, size=(ori_h // self.srf, ori_w // self.srf), mode=mode)
            cur_blur = filter2D(cur_blur, self.sinc_kernel)
        else:
            mode = random.choice(['area', 'bilinear', 'bicubic'])
            cur_blur = F.interpolate(cur_blur, size=(ori_h // self.srf, ori_w // self.srf), mode=mode)
            cur_blur = filter2D(cur_blur, self.sinc_kernel)
            cur_blur = self._jpge_compress(cur_blur, self.jpeg_range2)

        cur_blur = torch.clamp((cur_blur * 255.0).round(), 0, 255) / 255.

    # ----------------------- C) NOISE-ONLY ----------------------- #
        # add noise to the HR crop, then downscale with NEAREST (avoids blur)
        if self.usm:
            cur_noise = hr_usm
        else:
            cur_noise = hr

        cur_noise = self._add_noise(cur_noise, self.gray_noise_prob, self.gaussian_noise_prob,
                                    self.noise_range, self.poisson_scale_range)

        cur_noise = self._add_noise(cur_noise, self.gray_noise_prob2, self.gaussian_noise_prob2,
                                    self.noise_range2, self.poisson_scale_range2)

        # no blur, no JPEG, no sinc; just shrink to LR size without adding blur
        cur_noise = F.interpolate(cur_noise, size=(ori_h // self.srf, ori_w // self.srf), mode='nearest')
        cur_noise = torch.clamp((cur_noise * 255.0).round(), 0, 255) / 255.


        (hr, hr_usm), (lr_full, lr_blur, lr_noise, lr_first) = paired_random_crop([hr, hr_usm], [cur_full, cur_blur, cur_noise, cur_first], self.hr_size, self.srf)
        result['lr_full']  = lr_full.detach().clone()
        result['lr_blur']  = lr_blur.detach().clone()
        result['lr_noise'] = lr_noise.detach().clone()
        result['lr_first'] = lr_first.detach().clone()
        result['hr']       = hr.detach().clone()
        result['hr_usm']   = hr_usm.detach().clone()
...
class data_prefetcher():
    def __init__(self, loader, hr_size=256, srf=4, usm=False, need_l1=False):
        self.loader = loader
        self.loader_iter = iter(loader)
        self.stream = torch.cuda.Stream()
        self.degeration = Degeration(hr_size, srf, usm, need_l1)
        self.preload()
    ...
    def preload(self):
        try:
            ...
            deg_result = self.degeration(deg_info)

            self.next_hr     = deg_result['hr']
            self.next_hr_usm = deg_result['hr_usm']

            self.next_lr_full  = deg_result['lr_full']
            self.next_lr_blur  = deg_result['lr_blur']
            self.next_lr_noise = deg_result['lr_noise']
            self.next_lr_first = deg_result['lr_first']
        except StopIteration:
            self.next_hr     = None
            self.next_hr_usm = None

            self.next_lr_full  = None
            self.next_lr_blur  = None
            self.next_lr_noise = None
            self.next_lr_first = None
            return
        with torch.cuda.stream(self.stream):
            if self.next_hr is not None:
                self.next_hr     = self.next_hr.cuda(non_blocking=True)
                self.next_hr_usm = self.next_hr_usm.cuda(non_blocking=True)

                self.next_lr_full  = self.next_lr_full.cuda(non_blocking=True)
                self.next_lr_blur  = self.next_lr_blur.cuda(non_blocking=True)
                self.next_lr_noise = self.next_lr_noise.cuda(non_blocking=True)
                self.next_lr_first = self.next_lr_first.cuda(non_blocking=True)
            else:
                self.next_hr     = None
                self.next_hr_usm = None
    ...
    def next(self):
        ...
        lr_full  = self.next_lr_full
        lr_blur  = self.next_lr_blur
        lr_noise = self.next_lr_noise
        lr_first = self.next_lr_first
